...
import {updateItem} from "../../redux/Card"
...
function App({open,setopen,user,u}) {
...
u({ props: newr, type: "added" })
      firestore.collection("Posts").doc(`${number2}`).set(newr)
      .then(function() {
...
let nwa={
  ...
  uid:auth.currentUser.uid
    ...
    const number=Date.now()
    firestore.collection("Posts").doc(`${number}`).set(nwa)
    .then(function() {
        Alert.alert("successfully!");
